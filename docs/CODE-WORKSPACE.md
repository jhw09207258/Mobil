...
1~5번을 구현했다. 6번(Managed Agents)은 예고한 대로 제외했다.

| 항목 | 상태 |
|---|---|
| `code_repositories` 스키마 (0053) | 적용 완료 |
| GitHub 임포트 | 완료 — 브라우저에서, 트리 1회 + CDN N회 |
| 코드 저장소 UI · 대량 업로드 | 완료 |
| Monaco 교체 | 완료 — Yjs 협업 유지(`y-monaco`) |
| Claude 코드 어시스트 | 완료 — `/api/code-assist` |

### 설정이 필요한 것

코드 어시스트는 서버 환경변수 **`ANTHROPIC_API_KEY`** 를 쓴다. Vercel 프로젝트
설정에 넣으면 동작하고, 없으면 패널이 503 과 함께 "키가 설정되지 않았다"고
알려준다(조용히 실패하지 않는다).

모델은 `claude-opus-5`, adaptive thinking 을 쓴다. 선택 영역이 없으면 파일
전체를 보내며 6만 자를 넘으면 거절한다. 응답은 **패널에 보여주기만 하고 파일에
자동 반영하지 않는다** — 편집 중인 코드를 모델이 말없이 덮어쓰지 않게 하기 위함.

### Monaco 교체에서 걸렸던 것 (기록)

`monaco-editor` 의 `package.json` `exports` 가 `"./*" → "./esm/vs/*.js"` 로
매핑한다. 따라서 워커 지정자에 `esm/vs` 를 직접 쓰면 접두사가 두 번 붙어
해석에 실패한다 — `monaco-editor/editor/editor.worker.js` 가 맞는 형태다.
그리고 `y-monaco` 는 exports 필드 도입 이전에 작성돼 스스로
`monaco-editor/esm/vs/editor/editor.api.js` 를 임포트하므로, `next.config.mjs`
에서 그 지정자 하나만 실제 파일로 별칭을 걸었다(webpack·turbopack 양쪽).

### 남은 한계 (재확인)

- Python·Rust·Go 등은 **문법 강조만** 되고 자동완성은 안 된다. 3번 참조 —
  언어 서버를 상시 실행할 컨테이너가 있어야 가능하다.
- 터미널·디버거·확장은 브라우저에서 불가능하다.
- GitHub 임포트는 스냅샷 복사이며 git 히스토리는 오지 않는다.


---

## 7. 코드 에이전트 + Gemini (2026-07-26)

### Antigravity 는 연결할 수 없다 (확인함)

- Antigravity 는 **API 가 아니라 IDE**다. 공개 Gemini 엔드포인트를 쓰지 않고
  내부 엔드포인트(`daily-cloudcode-pa.sandbox.googleapis.com`)로 붙는다.
- **서드파티 도구로 Antigravity 에 접근하는 것은 Google 이용약관 위반이며
  계정 정지 사유**다. 이를 우회하는 프록시 프로젝트가 공개돼 있지만 같은
  이유로 쓰지 않는다.
- Google 자신이 "서드파티 코딩 에이전트에서 Gemini 를 쓰려면 **AI Studio 또는
  Vertex API 키**를 쓰라"고 안내한다 → 그대로 따랐다.

### Google AI Pro 구독은 API 를 포함하지 않는다 (확인함)

AI Pro($19.99/월)는 소비자용 구독(Gemini 앱 100회/일, Deep Research, 2TB 등)이고
**Gemini API 는 별도 과금**이다. 대신 **AI Studio 무료 티어**가 있어서
Flash 계열은 카드 없이 쓸 수 있다 — 이 기능에는 그것으로 충분하다.

키 발급: <https://aistudio.google.com/apikey> → 서버 환경변수 `GEMINI_API_KEY`.

### 구현

`/api/code-agent` — 모델: `gemini-3.5-flash`(Google 이 agentic·coding 용으로
안내하는 모델).

도구 두 개를 주고 **에이전트 루프**를 서버에서 돈다(최대 8스텝):

| 도구 | 용도 |
|---|---|
| `replace_in_file(old_string, new_string)` | 부분 치환. **파일에 정확히 한 번만** 나타날 때만 적용 |
| `rewrite_file(content)` | 전체 교체(대규모 재구성 전용) |

`old_string` 이 여러 번 나타나면 **실패시키고** 모델에게 더 넓은 문맥을 요구한다
— 첫 번째만 바꾸면 엉뚱한 위치를 조용히 고칠 수 있기 때문이다. 이 판정 로직은
`lib/apply-edit.ts` 로 분리해 테스트했다(유일/중복/부재/빈 문자열/공백 불일치
5가지).

편집 결과는 **Yjs 문서에 반영**한다 — 앞뒤 공통 부분을 제외한 최소 델타만
적용하므로 공동 편집자의 커서가 튀지 않고, 기존 자동 저장이 그대로 이어진다.

### 의도적으로 하지 않은 것

- **터미널 실행은 없다.** 브라우저에서 셸을 돌릴 수 없고, Vercel 서버리스에도
  영속 파일시스템이 없다(4번-(b) 참조). "명령을 실행하는" 에이전트가 필요하면
  Managed Agents 가 유일한 경로다.
- 에이전트는 **열려 있는 파일 하나만** 본다. 저장소 전체를 읽고 여러 파일을
  고치려면 컨텍스트 수집 + 다중 파일 편집이 필요하고, 그건 사실상 Managed
  Agents 영역이다.
