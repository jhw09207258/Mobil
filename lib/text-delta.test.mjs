...
assert.equal(textOf(mergeContentIntoSnapshot(null, "hello")), "hello");
...
// 기존 스냅샷 위에 얹으면 내용이 교체된다(이어붙지 않는다).
{
  ...
  assert.equal(textOf(merged), "updated text");
}
...
// 편집기가 업데이트를 받으면 내용이 수렴한다(협업 세션이 안 깨진다).
{
  ...
  Y.applyUpdate(live.doc, decodeUpdate(merged));
  assert.equal(live.toString(), "shared code + agent edit");
}

// 손상된 스냅샷은 버리고 새로 시드한다(터지지 않는다).
assert.equal(textOf(mergeContentIntoSnapshot("!!!not-base64!!!", "recovered")), "recovered");

// 빈 내용으로도 동작한다.
assert.equal(textOf(mergeContentIntoSnapshot(null, "")), "");

console.log("mergeContentIntoSnapshot: 5 checks passed");
