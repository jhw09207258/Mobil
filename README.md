...
| 테마 | 설정에서 액센트 색상 선택(프리셋 8종 + 자유 색상) — 배경 틴트·버튼·링크·활성 아이콘이 자동 파생(`lib/theme.ts`), 명도/채도 클램프로 어떤 색이든 가독성 유지, 액센트 명도에 따라 버튼 글자색 흑/백 자동(`--on-create`). localStorage 저장 + 루트 인라인 스크립트로 첫 페인트 전 적용(FOUC 없음) |
